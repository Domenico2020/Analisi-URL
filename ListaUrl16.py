...
parser.add_argument("-md", "--max_depth", help="Scrivere la profondità massima a cui arrivare", 
                    type=int, default=2)
parser.add_argument("-d", "--depth", help="Scrivere la profondità da cui partire", 
                    type=int, default=0)
...
class DFSCrawler():
    ...
    # il crawler
    def get_links_iteratively(self, base, path, visited, max_depth, depth):       
        if depth < max_depth:
            try:
...
for url in list_of_urls:
    ...
    print(' ')
    print(url)
    extractor.get_links_iteratively(url, "", set([url]), args.max_depth, args.depth)
